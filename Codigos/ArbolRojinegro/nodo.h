...
void insertarNodo (nodo *&arbol, int d, nodo *padre){
    if(arbol==NULL){
       ...
       if(d<valorRaiz){//si el elemente es menor a raiz, insertar a izq
        ...
        if(arbol->izq->padre->color==true){
            arbol->izq->color=false;//aca ya asigna el color negro a las hojas

        }
        else{
            arbol->izq->color=true;
        }

       }
       else{
        insertarNodo(arbol->der, d, arbol);
        if(arbol->der->padre->color==true){
            arbol->der->color=false;
        }
        else{
            arbol->der->color=true;
        }
       }
    }
